...
通用聊天框没有工作区；通用编程 Agent 有工作区，但界面、工具和默认行为都是给写代码的人准备的。本项目把同一套「能读写文件的 Agent」收成设计师的工作环境——中文文档树、设计文档原则、Ask / Plan / Agent 三档权限，并按文档编译提示词生成概念插画（图与提示词成对保存）。

- 后端：Python 3.11+ / FastAPI / LangGraph
- 前端：React 18 / TypeScript / Vite / Tailwind CSS 4
- 模型：文字用任意 OpenAI 兼容接口；生图按适配器（OpenAI Images、fal.ai，可再加）
...
- **文档旁概念插画**：从项目风格、当场加料和文档短描述编译提示词，生成概念图；记录不可变，可挂到文档 / 未绑定 / 草稿。文档更新后提示「未重画」，不自动重画

这版不做：生产用 2D 资产、音效试听、资产库、跨项目协作。生图模型在设置 → 图像模型里单独配置（与文字模型分开；按服务商协议走对应适配器）。
