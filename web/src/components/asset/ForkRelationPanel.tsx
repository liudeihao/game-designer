...
function nodeRow(n: ForkNode) {
  ...
  if (n.visibility === "deleted") {
    return (
      <p key={n.id} className="text-ui-mono text-[11px] text-text-muted/70 line-through">
        已删除的素材 <span className="font-mono text-[10px]">{n.id.slice(0, 8)}…</span>
      </p>
    );
  }
  return (
    <Link key={n.id} href={href} className="text-ui-mono block text-[11px] text-accent/90 hover:underline">
...
export function ForkRelationPanel({ assetId, className }: { assetId: string; className?: string }) {
  ...
  return (
    <div className={cn("space-y-3 border-t border-border/60 pt-3", className)}>
      <p className="text-ui-mono text-[10px] uppercase tracking-wider text-text-muted/80">Fork 关系</p>
      {up.length > 0 && (
        <div>
          <p className="text-ui-mono mb-1 text-[10px] text-text-muted">上游（直接来源）</p>
          <div className="space-y-1">{up.map(nodeRow)}</div>
        </div>
      )}
      {down.length > 0 && (
        <div>
          <p className="text-ui-mono mb-1 text-[10px] text-text-muted">下游（直接衍生）</p>
